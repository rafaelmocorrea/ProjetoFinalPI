...
'''
Metodo 2, usa o total de pixels brancos na imagem
...
def sumarizador_frames(path,limiar=20): #retorna o índice e máscaras dos frames de interesse
    ...
    while(1):
        ...
        soma_frame = mask.sum()
        if soma_frame > 0:
            soma_totais.append(soma_frame)
            frames_conteudo.append(mask)
            num_frames_conteudo.append(conta_frame)
    
    soma_totais = np.array(soma_totais)
    if len(num_frames_conteudo) > 0:
        print("Filtrando Frames")
        filtra_frames(soma_totais.mean(),frames_conteudo,num_frames_conteudo,limiar)
    video.release()
...
class Programa():
    def __init__(self,master = None):
        ...
        self.titulo = Label(self.cont1,text="Sumarizador de Vídeo: Método 2")
